...
def nodelta_compression5(file):
    ...
    for line in lines:
        tmp_data = eval(line[:-1])
        data = [int(x, base=16) for x in tmp_data]
        base_flit, offset_flits, delta_flits, mask = compresor_lib.adapted_no_delta_compressor(data)

        if len(delta_flits) >= 6:
            new_flits += 8
        else:
            new_flits += 1 + 1 + len(delta_flits)
...
def nodelta_compression6(file):
    ...
    for line in lines:
        tmp_data = eval(line[:-1])
        data = [int(x, base=16) for x in tmp_data]
        ok, base, compressed_offsets = compresor_lib.another_adapted_no_delta_compressor(data)
        if not ok:
            new_flits += 8
        else:
            new_flits += 2
